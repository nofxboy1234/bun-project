...
async function main() {
  const character = await db.query.characters.findFirst({
    where: (characters, { eq }) => eq(characters.id, 1),
    with: {
      ...
      species: true,
      contractsAsHuman: true,
      contractsAsDevil: true,
    },
  });

  // let characterContracts: (typeof contracts.$inferSelect)[] = [];
  let characterContracts: InferSelectModel<typeof contracts>[] = [];
  if (character) {
    if (character.species.name === "Human") {
      characterContracts = character.contractsAsHuman;
    } else if (character.species.name === "Devil") {
      characterContracts = character.contractsAsDevil;
    }
